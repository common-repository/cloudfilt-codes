...
/*
    Plugin Name: CloudFilt
    Plugin URI: https://cloudfilt.com/
    ...
    Version: 1.0.15 
    Author: CloudFilt    
    Author URI: https://cloudfilt.com/
...
class CloudFiltCodes {
    ...
    public function __construct() {
        ...
        add_action('init', [$this, 'disable_cache']);
    ...
    public function addBackendPHPCode() {
        ...
        if($found == 0){
            ...
            $body_CF = file_get_contents('php://input');
            $text20_CF = array();
            if (!empty($body_CF)) {
    		    $contentType = isset($_SERVER["CONTENT_TYPE"]) ? trim($_SERVER["CONTENT_TYPE"]) : '';
    		    if ($contentType === 'application/json') {
        		    $text20_CF = json_decode($body_CF, true);
    		    } elseif ($_SERVER['REQUEST_METHOD'] === 'POST') {
        		    parse_str($body_CF, $text20_CF);
    	        }
	        }
            
            $table_prefix = $wpdb->prefix;
            $authParams = array('log', 'pwd', 'password', 'pass');
            if (isset($text20_CF)){
            foreach ($text20_CF as $key => $value) {
                    if (strpos($key, $table_prefix) === 0) {
                        unset($text20_CF[$key]);
                    }
                    if (in_array($key, $authParams)) {
                        $text20_CF = array();
                        break;
                    }            
                }
